...
def generate_random_list(length: int):
	return [random.random()*10-5 for _ in range(length)]
...
def test_batched_conv():
	...
	input_vals = generate_random_list(reduce(lambda x, y: x*y, input_shape))
	weight_vals = generate_random_list(reduce(lambda x, y: x*y, weight_shape))
	input = neuroframe.Tensor(input_vals, [2, 3, 5, 5], neuroframe.float32, device_cuda)
	weight = neuroframe.Tensor(weight_vals, [2, 3, 3, 3], neuroframe.float32, device_cuda)
	output = neuroframe.ops.batched_convolution(input, weight)
	print(f"Input: {input}")
	print(f"Weight: {weight}")
	print(f"Output: {output}")

	input_th = torch.tensor(input_vals, dtype=torch.float32, device=torch.device("cuda:0")).reshape(input_shape)
	weight_th = torch.tensor(weight_vals, dtype=torch.float32, device=torch.device("cuda:0")).reshape(weight_shape)
...
def test_cross_entropy_loss():
	...
	input = neuroframe.Tensor(input_vals, input_shape, neuroframe.float32, device_cuda)
	ground_truth = neuroframe.Tensor(ground_truth_vals, ground_truth_shape, neuroframe.int32, device_cuda)
	...
	input_th = torch.tensor(input_vals, dtype=torch.float32, device=torch.device("cuda:0")).reshape(input_shape)
	ground_truth_th = torch.tensor(ground_truth_vals, dtype=torch.int64, device=torch.device("cuda:0")).reshape(ground_truth_shape)
...
def test_matmul_1():
	...
	input1_shape = [5, 6]
	input2_shape = [6, 7]
	input1_vals = generate_random_list(reduce(lambda x, y: x*y, input1_shape))
	input2_vals = generate_random_list(reduce(lambda x, y: x*y, input2_shape))
	input1 = neuroframe.Tensor(input1_vals, input1_shape, neuroframe.float32, device_cuda)
	input2 = neuroframe.Tensor(input2_vals, input2_shape, neuroframe.float32, device_cuda)
	output = neuroframe.ops.matmul(input1, input2)
	print(f"Input1: {input1}")
	print(f"Input2: {input2}")
	print(f"Output: {output}")

	input1_th = torch.tensor(input1_vals, dtype=torch.float32, device=torch.device("cuda:0")).reshape(input1_shape)
	input2_th = torch.tensor(input2_vals, dtype=torch.float32, device=torch.device("cuda:0")).reshape(input2_shape)
...
def test_tensor_binary_op():
	print(f"\n---- Tensor add:")
	input_shape = [3, 4]
	input1_vals = generate_random_list(reduce(lambda x, y: x*y, input_shape))
	input2_vals = generate_random_list(reduce(lambda x, y: x*y, input_shape))
	input1 = neuroframe.Tensor(input1_vals, input_shape, neuroframe.float32, device_cuda)
	input2 = neuroframe.Tensor(input2_vals, input_shape, neuroframe.float32, device_cuda)
	output = input1 + input2

	input1_th = torch.tensor(input1_vals, dtype=torch.float32, device=torch.device("cuda:0")).reshape(input_shape)
	input2_th = torch.tensor(input2_vals, dtype=torch.float32, device=torch.device("cuda:0")).reshape(input_shape)
