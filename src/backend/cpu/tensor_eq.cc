...
bool is_elem_eq(const T &a, const T &b) {
	...
	if constexpr (std::is_same_v<T, half>) {
		return abs_err <= HALF_ABS_THRES + HALF_REL_THRES * std::max(std::abs(a), std::abs(b));
	} else if constexpr (std::is_same_v<T, float>) {
		return abs_err <= FLOAT_ABS_THRES + FLOAT_REL_THRES * std::max(std::abs(a), std::abs(b));
	} else if constexpr (std::is_same_v<T, double>) {
		return abs_err <= DOUBLE_ABS_THRES + DOUBLE_REL_THRES * std::max(std::abs(a), std::abs(b));
	} else {
		return a == b;
	}
} 
